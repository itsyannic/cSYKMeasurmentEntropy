...
def on_shell_action_den(SD_Object: SchwingerDyson):
    ...
    term1 = -m/2*(np.log(arg) + 2*np.log(1+np.exp(SD_Object.mu*SD_Object.beta),dtype=np.double)) 
    term2 = -(1-m)*(np.log(np.linalg.det(np.linalg.inv(SD_Object.G33_d_free_inverse@SD_Object.G33d))) + 2*np.log(1+np.exp(SD_Object.mu*SD_Object.beta),dtype=np.double))
    ...
    term3 = -SD_Object.normalization*SD_Object.Jsqr*(1/SD_Object.q - 1)*np.trace(Gsqr@Gsqr)
...
def on_shell_action_num(SD_Object: SchwingerDyson):
    ...
    term1 = -m/2*(np.log(arg) + 2*np.log(1+np.exp(SD_Object.mu*SD_Object.beta),dtype=np.double)) 
    term2 = -(1-m)*(np.log(np.linalg.det(np.linalg.inv(SD_Object.G33_n_free_inverse@SD_Object.G33n))) + np.log(1+np.exp(SD_Object.mu*SD_Object.beta),dtype=np.double))
